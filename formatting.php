...
function backpress_pre_kses( $text ) {
	return preg_replace_callback('%<[^>]*?((?=<)|>|$)%', 'backpress_pre_kses_callback', $text);
}

function backpress_pre_kses_callback( $matches ) {
	if ( 1 !== substr_count($matches[0], '>') )
		return wp_specialchars($matches[0]);
	return $matches[0];
}

function backpress_sanitize_user( $username, $strict = false ) {
	$_username = $username;
	...
	// If strict, reduce for max portability.
	if ( $strict )
		$username = preg_replace('|[^a-z0-9-]|i', '', $username);

	return apply_filters( __FUNCTION__, $username, $_username, $strict );
}

function backpress_sanitize_slug( $text, $length = 0 ) {
	$_text = $text;
	$text = trim($text);
	...
	$text = apply_filters( 'pre_' . __FUNCTION__, $text, $_text, $length );
	...
	return apply_filters( __FUNCTION__, $text, $_text, $length );
}

function backpress_sanitize_slug_utf8( $text, $_text = '', $length = 0 ) {
...
function backpress_is_email( $email, $check_domain = false ) {
	...
	return apply_filters( __FUNCTION__, $r, $email );
}
