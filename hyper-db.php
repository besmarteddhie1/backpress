...
class HyperDB {
	 ...
	 * @param string $dbhost
	 */
	function HyperDB() { // ($dbuser, $dbpass, $dbname, $dbhost) {
		$args = func_get_args();
	...
	function __construct() { // ($dbuser, $dbpass, $dbname, $dbhost) {
		...
		if( empty($args) || defined( "WP_USE_MULTIPLE_DB" ) && CONSTANT( "WP_USE_MULTIPLE_DB" ) == true )
			return true;
		...
		if (!$this->dbh) {
			error_log( date( "Y-m-d H:i:s" ) . " Can't connect " . $dbhost." - $dbuser - $dbpass\n\n", 3, "/tmp/mysql.txt" );
		}
